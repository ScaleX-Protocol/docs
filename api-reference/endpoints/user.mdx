...
curl "https://api.scalex.money/api/account?address=0x1234..."
```

```javascript
const response = await fetch('https://api.scalex.money/api/account?address=0x1234...');
...
response = requests.get('https://api.scalex.money/api/account', {
    'address': '0x1234...'
})
...
curl "https://api.scalex.money/api/cross-chain-deposits?user=0x1234...&limit=50"
```

```javascript
const response = await fetch(
  'https://api.scalex.money/api/cross-chain-deposits?user=0x1234...&limit=50'
);
const deposits = await response.json();
```

```python
response = requests.get('https://api.scalex.money/api/cross-chain-deposits', {
    'user': '0x1234...',
    'status': 'RELAYED',
    'limit': 50
})
...
curl "https://api.scalex.money/api/token-mappings?sourceChainId=11155111&targetChainId=421614"
```

```javascript
const response = await fetch(
  'https://api.scalex.money/api/token-mappings?sourceChainId=11155111&targetChainId=421614'
);
...
async function getPortfolioValue(userAddress, prices) {
  const account = await fetch(
    `https://api.scalex.money/api/account?address=${userAddress}`
  ).then(r => r.json());
...
async function monitorDeposits(userAddress) {
  const deposits = await fetch(
    `https://api.scalex.money/api/cross-chain-deposits?user=${userAddress}&limit=10`
  ).then(r => r.json());
